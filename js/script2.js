...
const gejala = {
 ...
 // Menampilkan daftar gejala sebagai checkbox
const gejalaListEl = document.querySelector(".gejala-list");
Object.entries(gejala).forEach(([kode, data], index) => {
    const li = document.createElement("li");
    li.classList.add("gejala-item");
    li.innerHTML = `
        <label>
            <input type="checkbox" value="${kode}"> 
            <span class="gejala-text">${index + 1}. ${data.deskripsi}</span>
        </label>
    `;
    gejalaListEl.appendChild(li);
});

// Fungsi untuk menghitung diagnosa menggunakan CF dan FC
...
function tampilkanHasil(hasilCF, hasilFC) {
    ...
    if (hasilCF.length > 0 && hasilFC.length > 0) {
        ...
        const kesimpulanCF = hasilCF[0]; 
        const kesimpulanFC = hasilFC[0]; 
        ...
        resultEl.innerHTML = `
            <h3>Kesimpulan:</h3>
            Diagnosa terbaik adalah <strong>${solusiTerbaik.nama}</strong> berdasarkan metode <strong>${solusiTerbaik.metode}</strong> dengan nilai <strong>${solusiTerbaik.nilai}</strong>.<br>
            Rekomendasi solusi utama: <strong>${solusiTerbaik.solusi}</strong><br><br>
            <button id="detailBtn">Lihat Detail</button>
            <div id="detailHasil" style="display: none;">
                <h3>Hasil Diagnosa Certainty Factor:</h3>
                ${hasilCF.map(h => `<strong>${h.nama}</strong> - CF: ${h.CF}<br>Solusi: ${h.solusi}<br><br>`).join("")}
                <h3>Hasil Diagnosa Forward Chaining:</h3>
                ${hasilFC.map(h => `<strong>${h.nama}</strong> - Persentase: ${h.persentase}%<br>Solusi: ${h.solusi}<br><br>`).join("")}
            </div>
        `;

        // Event listener untuk tombol detail
        document.getElementById("detailBtn").addEventListener("click", function() {
            const detailEl = document.getElementById("detailHasil");
